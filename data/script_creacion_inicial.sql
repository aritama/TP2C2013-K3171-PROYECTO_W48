...
EXEC sp_defaultlanguage 'gd', 'spanish'
GO

IF SCHEMA_ID('PROYECTO_W') IS NOT NULL
BEGIN
		IF OBJECT_ID('PROYECTO_W.SP_CANCELAR_AGENDA') IS NOT NULL
			DROP PROCEDURE PROYECTO_W.SP_CANCELAR_AGENDA;

		IF OBJECT_ID('PROYECTO_W.SP_CANCELAR_TURNOS_POR_AGENDA_DESHABILITADA') IS NOT NULL
			DROP PROCEDURE PROYECTO_W.SP_CANCELAR_TURNOS_POR_AGENDA_DESHABILITADA;
			
		IF OBJECT_ID('PROYECTO_W.SP_CANCELAR_AGENDA') IS NOT NULL
			DROP PROCEDURE PROYECTO_W.SP_CANCELAR_AGENDA;
			
		IF OBJECT_ID('PROYECTO_W.F_TIMEDIFF_PROF') IS NOT NULL
			DROP FUNCTION PROYECTO_W.F_TIMEDIFF_PROF;
			
		IF OBJECT_ID('PROYECTO_W.TR_48HORAS_SEMANALES') IS NOT NULL
			DROP TRIGGER PROYECTO_W.TR_48HORAS_SEMANALES;
			
		IF OBJECT_ID('PROYECTO_W.F_48HORAS_CHECK') IS NOT NULL
			DROP FUNCTION PROYECTO_W.F_48HORAS_CHECK;
...
GO
	...
	-- CALCULA DIFERENCIA ENTRE HORARIOS DE UN RANGO HORARIO DE UN PROFESIONAL
CREATE FUNCTION PROYECTO_W.F_TIMEDIFF_PROF
(@INICIO TIME, @FIN TIME)
RETURNS NUMERIC(3,1)
BEGIN
RETURN (SELECT (DATEPART(HH,@FIN)-DATEPART(HH,@INICIO)) + (
		CASE
			WHEN ((DATEPART(MI,@FIN)-DATEPART(MI,@INICIO))=30)
				THEN 0.5
			WHEN ((DATEPART(MI,@FIN)-DATEPART(MI,@INICIO))=-30)
				THEN -0.5
			WHEN ((DATEPART(MI,@FIN)-DATEPART(MI,@INICIO))=0)
				THEN 0
		END)
		)
END
GO 
	...
	-- AVISA SI SE HA SOBREPASADO 48 HORAS DE CARGA SEMANAL EN LOS RANGOS INGRESADOS
CREATE FUNCTION PROYECTO_W.F_48HORAS_CHECK
(@PROF_DNI NUMERIC(18,0), @DESDE DATE, @HASTA DATE)
RETURNS TABLE
RETURN (
	SELECT DATEPART(WEEK,hora_fecha) AS SEMANA, SUM(PROYECTO_W.F_TIMEDIFF_PROF(hora_inicio,hora_fin)) AS SUMA
	FROM PROYECTO_W.RangoHorario
	JOIN PROYECTO_W.Fecha ON fecha_agen_cod = hora_agen_cod AND hora_fecha = fecha_fecha
	...
	GROUP BY DATEPART(WEEK,hora_fecha)
	HAVING SUM(PROYECTO_W.F_TIMEDIFF_PROF(hora_inicio,hora_fin)) > 48
	)
GO
...
GO
	...
	-- CONTROLA 48 HORAS SEMANALES DEL PROFESIONAL
/*
CREATE TRIGGER PROYECTO_W.TR_48HORAS_SEMANALES
ON PROYECTO_W.RangoHorario
FOR INSERT
AS
IF ((SELECT COUNT(DISTINCT DATEPART(WEEK,hora_fecha)) FROM inserted)>0)
BEGIN
	SELECT DATEPART(WEEK,INS.hora_fecha),INS.hora_agen_cod,SUM(PROYECTO_W.F_TIMEDIFF_PROF(RH.hora_inicio,RH.hora_fin))
	FROM PROYECTO_W.RangoHorario AS RH
	JOIN inserted AS INS ON DATEPART(WEEK,INS.hora_fecha) = DATEPART(WEEK,RH.hora_fecha)
	WHERE INS.hora_agen_cod = RH.hora_agen_cod
	GROUP BY DATEPART(WEEK,INS.hora_fecha),INS.hora_agen_cod
	HAVING SUM(PROYECTO_W.F_TIMEDIFF_PROF(RH.hora_inicio,RH.hora_fin)) > 48
	
	IF((SELECT ROWCOUNT_BIG()) > 0)
	BEGIN
		RAISERROR('48 HORAS SEMANALES SUPERADAS',16,1)
